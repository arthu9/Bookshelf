...
function addbookisbn(isbn) {
   ...
   app.request({
      url: 'hhttp://127.0.0.1:5050/mobile/user/isbn_check/' + isbn,
      contentType: 'application/json; charset=utf-8',
      method: "GET",
      dataType: "json",
      crossDomain: true,
  ...
  app.request({
    ...
    success: function (data) {
      ...
      for (var i = 0; i < data.book.length; i++) {
        ...
        if(data.book[j].title == data.book[i].title){
          j = j + 1
        }else{
        employee_data += '<div class="swiper-slide">';
        employee_data += '<a onclick="onebook(); bookindex(\'' + data.book[i].book_id + '\'); bookowner(\'' + data.book[i].owner_username + '\')" href="/item/">';
...
function onebook() {
  ...
  app.request({
    ...
    success: function (data) {
      ...
      //   owner += '<ul>';
      //   owner += '<li>';
      //   owner += '<a href="#">Name:' + data.book[i].owner+'</a>';
      //   owner += '</li>';
      //   owner += '</ul>';  
      // }
      // $('#owner').append(owner);
...
function owneronebook() {
  ...
  loops = [];
  app.request({
    url: 'https://desolate-basin-69053.herokuapp.com/user/bookshelf/bookdb',
    method: "POST",
    contentType: 'application/json; charset=utf-8',
    headers: { 'x-access-token': tokens },
    dataType: "json",
